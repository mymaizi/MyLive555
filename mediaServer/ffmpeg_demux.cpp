...
u_int8_t FfmpegDemux::Parse() {
    ...
    do {
        ...
        if (ReadOneFrame(&packet, has_extra_data) == 0) {
            ...
            AVCodecParameters* codec = format_ctx_->streams[stream_id]->codecpar;
...
int FfmpegDemux::ParseH264ExtraDataInMp4(AVPacket* packet,int stream_id) {
#ifdef NOT_USE_BITSTREAM_FILER
    ...
    avcodec_parameters_copy(absCtx->par_in, codec);
    av_bsf_init(absCtx);
    av_bsf_send_packet(absCtx, packet);
    av_bsf_receive_packet(absCtx, packet);
    av_bsf_free(&absCtx);
#endif
    return 0;
}
...
int FfmpegDemux::ReadOneFrame(AVPacket* packet, boolean& has_extra_data) {
    int stream_id = -1;
    char const* extension = strrchr(format_ctx_->url, '.');
    ...
    if (strcmp(extension, ".mp4") == 0) {
        ...
        codec = format_ctx_->streams[stream_id]->codecpar;
